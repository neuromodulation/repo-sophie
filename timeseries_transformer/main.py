...
# evtl imputation?only one that uses unsupervised runner (paper how much is unsupervised!)
...
def main(config):
    ...
    if config['task'] == 'classification':
        ...
        labels = data_class.labels_df.values.flatten()
    else:
    ...
    if config['val_ratio'] > 0:
        train_indices, val_indices, test_indices = split_dataset(data_indices=data_class.all_IDs,
        ...
        train_indices = data_class.all_IDs
    ...
    if config['norm_from']:
        ...
        data_class.feature_df.loc[train_indices] = normalizer.normalize(data_class.feature_df.loc[train_indices])
        #data_class.feature_df.loc[data_class.feature_df["FileID"].isin(train_indices)] = normalizer.normalize(data_class.feature_df.loc[data_class.feature_df["FileID"].isin(train_indices)])
    ...
    if normalizer is not None:
        if len(val_indices):
            ...
            # val_data.feature_df.loc[val_data.feature_df["FileID"].isin(val_indices)] = normalizer.normalize(val_data.feature_df.loc[val_data.feature_df["FileID"].isin(val_indices)])        
        if len(test_indices) != 0:
            test_data.feature_df.loc[test_indices] = normalizer.normalize(test_data.feature_df.loc[test_indices])
            # test_data.feature_df.loc[test_data.feature_df["FileID"].isin(test_indices)] = normalizer.normalize(test_data.feature_df.loc[test_data.feature_df["FileID"].isin(test_indices)])        

#####################################

    # Create model
    logger.info("Creating model ...")
    model = model_factory(config, data_class)
    ...
    # val_dataset = val_data.data[val_data.data["FileID"].isin(val_indices)]
######################################################################################################    
    ...
    train_dataset = dataset_class(data_class, train_indices)
############################################################################################
    ...
    for epoch in range(start_epoch + 1, config["epochs"] + 1): # in tqdm(range(start_epoch etc.), desc='Training Epoch', leave=False:
