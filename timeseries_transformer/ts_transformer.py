...
def get_pos_encoder(pos_encoding):
    ...
    # args = Options().parse()  # `argsparse` object
    # config = setup(args)
    def init_pos_encoder(d_model, max_len=1024, dropout=0.1):
        if pos_encoding == "learnable":
            return LearnablePositionalEncoding(d_model=d_model, max_len=1024, dropout=dropout)
        elif pos_encoding == "fixed":
            return FixedPositionalEncoding(d_model=d_model, dropout=dropout, max_len=max_len)
        else:
            raise NotImplementedError("pos_encoding should be 'learnable'/'fixed', not '{}'".format(pos_encoding))
    return init_pos_encoder    
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
        ...
        self.project_inp = nn.Linear(feat_dim, d_model)
        self.pos_enc = get_pos_encoder(pos_encoding)(d_model=d_model, max_len=max_len)
