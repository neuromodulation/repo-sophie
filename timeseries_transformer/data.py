...
import sktime
# from sktime.utils import load_from_tsfile_to_dataframe
...
class TSRegressionArchive(BaseData):
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        if pattern is None:
            ...
            selected_paths = data_paths # list(filter(lambda x: re.search(pattern, x), data_paths))

        input_paths = [p for p in selected_paths if os.path.isfile(p) and p.endswith('.vdhr')]
        if len(input_paths) == 0:
            raise Exception("No .vdhr files found using pattern: '{}'".format(pattern))
    ...
    def load_single(self, filepath):
        ...
        if self.config['task'] == 'regression':
            ...
            df = pd.DataFrame(raw, dtype=np.float32)
            ####################################################################################################################################################################
